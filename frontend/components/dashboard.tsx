...
import {
  ArrowLeft, BarChart3, Box, Check, ChevronDown, ChevronFirst, ChevronLast, ChevronLeft, ChevronRight, CircleHelp, Clock3, FileText, FileVideo, GitCompareArrows, LogOut, Megaphone,
  CalendarDays, Cpu, FolderCog, Gauge, ImageIcon, MapPinned, Menu, MessageSquareText, MoreHorizontal, Plus, Presentation, RotateCcw, ScanLine, Search, Settings2, ShieldCheck, Trash2, UserCircle, Waves, X,
} from "lucide-react";
...
export function Dashboard({
  ...
  useEffect(() => {
    function restoreWorkspaceLocation() {
      ...
      const next = params.get("workspace") as WorkspaceView | null;
  ...
  function navigate(next: WorkspaceView) {
    setProfilePanelOpen(false);
    if (next === "records") setSelected(null);
    setView(next);
  ...
  return <div className={`auth-entry-shell analysis-entry-shell analysis-center-shell analysis-center-${view}`}>
    ...
    <header className="auth-topline analysis-workspace-topline">
      ...
      <nav className="auth-top-menu" aria-label="상단 메뉴" onMouseLeave={() => setOpenMenu(null)}>
        ...
        <div className={`auth-menu-group ${openMenu === "analysis" ? "menu-open" : ""}`} onMouseEnter={() => setOpenMenu("analysis")}>
          ...
          <div><button type="button" onClick={() => { setOpenMenu(null); navigate("realtime"); }}>실시간 탐색</button><button type="button" onClick={() => { setOpenMenu(null); navigate("analysis"); }}>부유물 탐색</button><button type="button" onClick={() => { setOpenMenu(null); navigate("records"); }}>탐색 기록</button><button type="button" onClick={() => { setOpenMenu(null); navigate("compare"); }}>AI 성능 비교</button></div>
        </div>
        <div className={`auth-menu-group ${openMenu === "board" ? "menu-open" : ""}`} onMouseEnter={() => setOpenMenu("board")}>
          ...
          <div><button type="button" onClick={() => { setOpenMenu(null); navigate("notice"); }}>공지사항</button><button type="button" onClick={() => { setOpenMenu(null); navigate("free"); }}>자유게시판</button><button type="button" onClick={() => { setOpenMenu(null); navigate("bug"); }}>버그 제보</button><button type="button" onClick={() => { setOpenMenu(null); navigate("faq"); }}>자주 묻는 질문</button></div>
        </div>
      </nav>
      ...
      <div key={`${view}-${viewRevision}`} className={`${isStoryView ? "workspace-body story-workspace-body public-shell public-view-" + view : "workspace-body"} page-content-transition`}>
        {view === "analysis" && <>
          <section className={`analysis-upload-workspace analysis-result-modal analysis-explorer-workbench ${displayedAnalysisId ? "has-result" : ""}`}>
            <header className="analysis-result-modal-header explorer-hero"><div><p>AI OBSERVATION WORKBENCH</p><h2>부유물 탐색</h2><span>{displayedAnalysisId ? "진행 중이거나 완료된 분석을 이어서 확인합니다." : "관측 미디어에 AI 모델을 적용해 부유물의 위치와 종류를 탐색합니다."}</span></div>{displayedAnalysisId && currentAnalysis && !(["queued", "processing"] as Analysis["status"][]).includes(currentAnalysis.status) ? <button className="secondary-button explorer-new-analysis-button" type="button" onClick={prepareNewAnalysis}><Plus size={15}/>새 탐색 준비</button> : <div className="explorer-status"><i/><span>{displayedAnalysisId ? "진행 중 작업 복원됨" : "분석 시스템 준비됨"}</span></div>}</header>
            {!displayedAnalysisId && <form onSubmit={start}>
              <div className="explorer-steps" aria-label="분석 준비 단계">
                ...
                <span className={selectedModel ? "complete" : selectedVideo ? "active" : ""}><b>{selectedModel ? <Check size={14}/> : "2"}</b>모델 연결</span>
                <span className={selectedVideo && selectedModel ? "active" : ""}><b>3</b>분석 실행</span>
              </div>
              <div className="explorer-grid">
                <section className="explorer-preview-panel">
                  ...
                  <div className={`analysis-inline-preview ${selectedVideo ? "has-media" : ""}`}>{selectedVideo ? selectedVideo.media_type === "image" ? <div className="analysis-preview-image-frame"><ZoomableImage src={`${API_URL}/videos/${selectedVideo.id}/preview`} alt={`${selectedVideo.name} 미리보기`} imageStyle={{ width: "100%", height: "100%", maxWidth: "100%", maxHeight: "100%", objectFit: "contain", objectPosition: "center", display: "block" }}/></div> : <video src={`${API_URL}/videos/${selectedVideo.id}/preview`} controls preload="metadata"/> : <div><span className="preview-empty-icon"><ImageIcon size={30}/></span><strong>분석 대상을 선택하세요</strong><span>JPG, JPEG, PNG, WEBP, BMP · MP4, AVI, MOV, MKV, WEBM 지원</span><button type="button" onClick={() => openUpload("video")}><Plus size={14}/>미디어 업로드</button></div>}</div>
                  {selectedVideo && <div className="media-facts"><span><small>형식</small><strong>{selectedVideo.media_type === "image" ? "IMAGE" : "VIDEO"}</strong></span><span><small>크기</small><strong>{formatBytes(selectedVideo.size_bytes)}</strong></span><span><small>길이</small><strong>{formatDuration(selectedVideo.duration_seconds)}</strong></span><span><small>프레임</small><strong>{selectedVideo.frame_count?.toLocaleString() ?? "—"}</strong></span></div>}
                ...
                <aside className="explorer-control-panel">
                  <section><div className="explorer-section-heading"><div><span>02 · AI MODEL</span><h3>탐지 모델</h3></div><div className="explorer-heading-actions"><button type="button" onClick={() => openAssetManager("model")}><FolderCog size={14}/>관리</button><button type="button" onClick={() => openUpload("model")}><Plus size={14}/>새 모델</button></div></div><AnalysisPicker name="model" label="적용 모델" placeholder="YOLO PT 모델을 선택하세요" icon={<Cpu size={18}/>} value={selectedModelId} open={openPicker === "model"} onOpen={() => setOpenPicker(openPicker === "model" ? null : "model")} onClose={() => setOpenPicker(null)} onChange={setSelectedModelId} options={models.map((item) => ({ value: String(item.id), label: item.name, meta: `${item.task ?? "Detection"} · ${formatBytes(item.size_bytes)}` }))}/>{selectedModel && <div className="model-summary"><span><Box size={17}/></span><div><strong>{selectedModel.task ?? "Object Detection"}</strong><small>{selectedModel.class_names.length ? `${selectedModel.class_names.length}개 탐지 클래스` : "클래스 정보는 분석 시 확인"}</small></div><Check size={16}/></div>}</section>
                  <section className="explorer-settings"><div className="explorer-section-heading"><div><span>03 · PARAMETERS</span><h3>탐지 설정</h3></div><Settings2 size={18}/></div><label><span><strong>최소 신뢰도</strong><output>{Math.round(confidence * 100)}%</output></span><input name="confidence" type="range" min="0.1" max="0.9" step="0.05" value={confidence} onChange={(event) => setConfidence(Number(event.target.value))}/><small>낮을수록 더 많은 후보를 탐지합니다.</small></label><label><span><strong>프레임 간격</strong><output>{frameStride} frame</output></span><input name="stride" type="range" min="1" max="10" step="1" value={frameStride} onChange={(event) => setFrameStride(Number(event.target.value))}/><small>간격이 작을수록 정밀하지만 오래 걸립니다.</small></label></section>
                  <div className={`explorer-readiness ${selectedVideo && selectedModel ? "ready" : ""}`}><Gauge size={18}/><div><strong>{selectedVideo && selectedModel ? "분석 준비 완료" : "분석 준비 중"}</strong><span>{!selectedVideo ? "관측 미디어를 선택해 주세요." : !selectedModel ? "탐지 모델을 선택해 주세요." : "설정을 확인하고 분석을 시작하세요."}</span></div></div>
                </aside>
              </div>
              <div className="analysis-run"><div><small>ESTIMATED WORKFLOW</small><p>{analysisSubmitting ? "분석 요청을 등록하고 있습니다." : analysisRunning ? "현재 분석이 진행 중입니다. 완료 후 새 분석을 시작할 수 있습니다." : selectedVideo && selectedModel ? `${selectedVideo.name}에 ${selectedModel.name} 모델을 적용합니다.` : "미디어와 AI 모델을 연결하면 실행할 수 있습니다."}</p></div><button className="primary-button" disabled={!selectedModelId || !selectedVideoId || analysisRunning || analysisSubmitting}><ScanLine size={18}/>{analysisSubmitting ? "분석 요청 중" : analysisRunning ? "분석 진행 중" : "AI 탐색 시작"}<ChevronRight size={17}/></button></div>
            </form>}
            ...
            {displayedAnalysisId && <div ref={resultSectionRef} className="analysis-result-modal-body analysis-embedded-result" aria-live="polite"><AnalysisDetail id={displayedAnalysisId} onUpdated={refresh} onOpenDetails={openAnalysisRecord} onAnalysisCreated={(id) => { setCurrentAnalysisId(id); setSelected(id); window.history.replaceState({ workspace: "analysis", analysis: id }, "", `/auth?workspace=analysis&analysis=${id}`); }} onPrepareNew={prepareNewAnalysis} showReportSummary={false}/></div>}
          </section>
        </>}
        ...
        {view === "records" && <section className={`records-workbench ${selected ? "records-detail-view" : "records-list-view"}`}>
          {!selected ? <>
            ...
            <div className="records-detail-content"><AnalysisDetail id={selected} onUpdated={refresh} onAnalysisCreated={selectAnalysisRecord} onPrepareNew={() => navigate("analysis")}/></div>
          </main>}
        </section>}

        {view === "compare" && <AnalysisComparison analyses={analyses} onStartAnalysis={() => navigate("analysis")}/>}
        {!(["home", "analysis", "realtime", "records", "compare"] as WorkspaceView[]).includes(view) && <WorkspaceSection key={`${view}-${viewRevision}`} view={view as Exclude<WorkspaceView, "home" | "analysis" | "realtime" | "records" | "compare">} user={user} onNavigate={navigate}/>}
      </div>
    </div>
    </section>
    </main>
    {profilePanelOpen && <><button className={`analysis-profile-backdrop ${profilePanelClosing ? "is-closing" : ""}`} type="button" aria-label="마이페이지 닫기" onClick={closeMyPage}/><aside className={`auth-panel analysis-profile-drawer ${profilePanelClosing ? "is-closing" : ""}`} aria-label="마이페이지">
      <div className="auth-form-wrap"><div className="profile-panel-content">
        ...
        {!profileEditing && <div className="profile-view-enter"><div className="profile-identity"><span>{user.name.slice(0, 1)}</span><div><strong>{user.name}님</strong><p>{user.email}</p></div>{user.role === "admin" ? <div className="profile-admin-actions"><button className="profile-admin-badge" type="button" onClick={() => navigate("admin")} title="관리자 페이지로 이동"><ShieldCheck size={11}/>관리자</button><button className="profile-admin-badge" type="button" onClick={() => { window.location.href = "/presentation"; }} title="PPT 페이지로 이동"><Presentation size={11}/>PPT</button></div> : <em><ShieldCheck size={11}/>일반 회원</em>}</div>
