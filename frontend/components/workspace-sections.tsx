...
import type { AdminUser, Analysis, ContentItem, Inquiry, User } from "@/lib/types";
...
function InquirySection({ user }: { user: User }) {
  ...
  if (writing) return <div className="section-page inquiry-compose-page"><header><button type="button" onClick={() => setWriting(false)}><ArrowLeft size={17}/>문의 목록</button><div><p className="section-kicker">PRIVATE SUPPORT</p><h2>1:1 문의 작성</h2><p>서비스 이용 중 확인이 필요한 내용을 남겨주세요.</p></div><span><ShieldCheck size={17}/>비공개 문의</span></header><form onSubmit={submit}><label><span>문의 제목</span><input name="title" required minLength={2} maxLength={120} placeholder="문의 내용을 한 문장으로 입력하세요"/></label><label><span>문의 내용</span><textarea name="content" rows={10} required minLength={5} maxLength={5000} placeholder="확인이 필요한 상황과 요청 사항을 구체적으로 작성해주세요."/></label><div className={`inquiry-attachment-picker ${preview ? "has-preview" : ""}`}>{preview && <img src={preview} alt="첨부 이미지 미리보기"/>}<label><Paperclip size={19}/><span><strong>{attachment ? attachment.name : "파일 첨부"}</strong><small>{attachment ? `${formatBytes(attachment.size)} · 다른 파일을 선택하려면 클릭하세요` : "이미지와 문서를 첨부할 수 있습니다. 최대 20MB"}</small></span><input type="file" onChange={(event) => setAttachment(event.target.files?.[0] ?? null)}/></label></div><footer><p><ShieldCheck size={14}/>작성한 문의와 첨부파일은 본인과 관리자만 확인할 수 있습니다.</p><div><button type="button" onClick={() => setWriting(false)}>취소</button><button className="primary-button"><Send size={16}/>문의 등록</button></div></footer></form></div>;
  return <div className="section-page inquiry-page"><header className="inquiry-hero"><div><span><Inbox size={22}/></span><div><p className="section-kicker">MY INQUIRIES</p><h2>1:1 문의</h2><p>문의 진행 상태와 관리자 답변을 한곳에서 확인합니다.</p></div></div><button className="primary-button" onClick={() => setWriting(true)}><PenLine size={16}/>문의 작성</button></header><div className="inquiry-summary"><div><small>전체 문의</small><strong>{items.length}<em>건</em></strong></div><div><small>답변 대기</small><strong>{items.filter((item) => item.status !== "answered").length}<em>건</em></strong></div><p><ShieldCheck size={14}/>문의 내용은 비공개로 보호됩니다.</p></div><div className="inquiry-list">{items.map((item) => <article key={item.id} className={open === item.id ? "open" : ""}><button onClick={() => setOpen(open === item.id ? null : item.id)}><span className={`status ${item.status === "answered" ? "completed" : "processing"}`}>{item.status === "answered" ? "답변 완료" : "답변 대기"}</span><strong>{item.title}</strong><time>{formatDate(item.created_at)}</time><ChevronDown size={18}/></button>{open === item.id && <div className="inquiry-body"><div><small>문의 내용</small><p>{item.content}</p>{item.attachments?.length > 0 && <InquiryAttachments files={item.attachments}/>}</div>{item.answer ? <section><b>관리자 답변</b><p>{item.answer}</p></section> : <aside>관리자가 문의 내용을 확인하고 있습니다.</aside>}</div>}</article>)}{!items.length && <div className="inquiry-empty"><Inbox size={28}/><strong>등록한 문의가 없습니다.</strong><p>궁금한 내용이 있다면 문의를 작성해주세요.</p><button onClick={() => setWriting(true)}>첫 문의 작성</button></div>}</div></div>;
}
...
function AdminConsole() {
  ...
  async function load() {
    ...
    if (tab === "records") setRecords(await api<(Analysis & { owner: { id: number; name: string } })[]>("/admin/analyses"));
  ...
  async function updateUser(item: AdminUser, changes: Partial<AdminUser>) {
    ...
    try {
      await api(`/admin/users/${item.id}`, { method: "PATCH", body: JSON.stringify(changes) });
  ...
  async function deleteRecord(id: number) { if (!confirm("분석 기록을 삭제하시겠습니까?")) return; await api(`/admin/analyses/${id}`, { method: "DELETE" }); await load(); }
  async function answerInquiry(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!selectedInquiry) return; const data = new FormData(event.currentTarget); await api(`/inquiries/${selectedInquiry.id}/answer`, { method: "PATCH", body: JSON.stringify({ answer: data.get("answer") }) }); await load(); }
  ...
  return <div className="admin-page admin-console-page"><header className="admin-console-intro"><div><p className="section-kicker">SERVICE CONTROL</p><h2>서비스 운영 관리</h2><p>회원 상태와 분석 로그, 비공개 문의를 확인하고 관리합니다.</p></div><span>{itemCount}</span></header><div className="admin-tabs"><button className={tab === "users" ? "active" : ""} onClick={() => setTab("users")}><UserCog size={17}/><span><strong>회원 관리</strong><small>권한 및 이용 상태</small></span></button><button className={tab === "records" ? "active" : ""} onClick={() => setTab("records")}><Video size={17}/><span><strong>로그 관리</strong><small>전체 분석 이력</small></span></button><button className={tab === "inquiries" ? "active" : ""} onClick={() => setTab("inquiries")}><MessageSquareText size={17}/><span><strong>1:1 문의 관리</strong><small>문의 확인 및 답변</small></span></button></div>
    ...
    {tab === "records" && <div className="admin-table"><div className="admin-row record head"><span>사용자</span><span>분석 미디어</span><span>적용 모델</span><span>상태</span><span>관리</span></div>{records.map((item) => <div className="admin-row record" key={item.id}><strong>{item.owner.name}</strong><span>{item.video.name}</span><span>{item.model.name}</span><span className={`status ${item.status}`}>{({ queued: "대기", processing: "분석 중", completed: "완료", failed: "실패" })[item.status]}</span><button className="table-icon" aria-label="분석 로그 삭제" title="분석 로그 삭제" onClick={() => deleteRecord(item.id)}><Trash2 size={15}/></button></div>)}{!records.length && <div className="admin-empty">저장된 분석 로그가 없습니다.</div>}</div>}
    {tab === "inquiries" && <section className="admin-inquiry-list"><header><div><strong>접수된 문의</strong><span>문의 내용을 확인하고 같은 화면에서 답변합니다.</span></div><b>{inquiries.filter((item) => item.status !== "answered").length}건 대기</b></header>{inquiries.map((item) => { const expanded = selectedInquiry?.id === item.id; return <article className={expanded ? "open" : ""} key={item.id}><button type="button" onClick={() => setSelectedInquiry(expanded ? null : item)}><span className={`status ${item.status === "answered" ? "completed" : "processing"}`}>{item.status === "answered" ? "답변 완료" : "답변 대기"}</span><strong>{item.title}</strong><small>{item.user.name} · {item.user.email}</small><time>{formatDate(item.created_at)}</time><ChevronDown size={17}/></button>{expanded && <div className="admin-inquiry-detail"><div className="admin-inquiry-question"><small>문의 내용</small><p>{item.content}</p>{item.attachments?.length > 0 && <InquiryAttachments files={item.attachments}/>}</div><form onSubmit={answerInquiry}><label><span>관리자 답변</span><textarea name="answer" rows={6} required defaultValue={item.answer ?? ""} placeholder="회원에게 전달할 답변을 작성하세요."/></label><button className="primary-button"><Send size={15}/>{item.answer ? "답변 수정" : "답변 등록"}</button></form></div>}</article> })}{!inquiries.length && <div className="admin-empty">접수된 문의가 없습니다.</div>}</section>}
  </div>;
}
